...
public class GraphResultSet implements Iterable<GraphResult> {
    ...
    private static final Function<Row, GraphResult> TO_GRAPH_RESULT = new Function<Row, GraphResult>() {
        @Override
        public GraphResult apply(Row row) {
            String jsonString = row.getString("gremlin");
            try {
                ObjectMapper mapper = GraphUtils.OBJECT_MAPPER;
                JsonNode jsonNode = mapper.readTree(jsonString).get("result");
    ...
    public GraphResult one() {
        return TO_GRAPH_RESULT.apply(wrapped.one());
    }
    ...
    public List<GraphResult> all() {
        return Lists.transform(wrapped.all(), TO_GRAPH_RESULT);
    }
