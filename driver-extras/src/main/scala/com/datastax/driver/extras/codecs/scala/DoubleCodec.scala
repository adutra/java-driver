...
object DoubleCodec extends TypeCodec[Double](DataType.cdouble(), new TypeToken[Double]() {}) {
  ...
  override def parse(value: String): Double = {
    try {
      ...
      case e: NumberFormatException =>
        throw new InvalidTypeException( s"""Cannot parse 64-bits double value from "$value"""")
    }
  }
