...
object FloatCodec extends TypeCodec[Float](DataType.cfloat(), new TypeToken[Float]() {}) {
  ...
  override def parse(value: String): Float = {
    try {
      ...
      case e: NumberFormatException =>
        throw new InvalidTypeException( s"""Cannot parse 32-bits float value from "$value"""")
    }
  }
